...
class ReRanker:
    ...
    def __init__(self, opts):
        ...
        if self.rerank_type == 'cross' or self.rerank_type == 'cross_tune':
            if self.rerank_type == 'cross':
                ...
                # DÜZELTME: Eğer girilen checkpoint yolu sistemde mevcutsa doğrudan kullan (Dinamik yol desteği)
                if os.path.exists(opts.reranker_checkpoint):
                    self.reranker_checkpoint = opts.reranker_checkpoint
                else:
    ...
    def load_user(self, opts):
        # DÜZELTME: Sabit 'dev' yerine opts.data_split kullanarak esneklik sağlandı
        opts.user_vocab_path = os.path.join(opts.data_addr,
                                            f"{opts.data_split}/{opts.source}")
        vocab_addr = opts.user_vocab_path

        # DÜZELTME: Sabit 'dev' yerine opts.data_split kullanarak esneklik sağlandı
        opts.user_emb_path = os.path.join(opts.data_addr,
                                          f"{opts.data_split}/{opts.source}/user_emb",
                                          opts.user_emb_path)
